...
-- define the model's type and structure
...
-- pass that in so we can initalize the state of the first model
...
type Msg
    = Increment
    | Decrement
    | Reset -- two messages possible we will do something diferent for each


update :
    Msg
    -> Model
    -> Model -- no idea what this structure is....
update msg model =
    case msg of
        Increment ->
            -- if we get a n Increment message
            model + 1

        -- add one to the model which is just a counter
        Decrement ->
            -- if we get a dec message
            model - 1

        -- subtract from the model
        Reset ->
            0



-- set the model to the innitial value
-- VIEW
