...
module Main exposing (Model, Msg(..), childNav, displayTree, enterAndGo, goBackUp, goNextDn, init, last_child_id, main, makeChildButton, subscriptions, update, view)
    ...
    case msg of
        ...
        KeyboardMsg keyMsg ->
            let
                ...
                ctl_back =
                    Keyboard.isPressed Keyboard.ArrowLeft keyboardModel
                        && Keyboard.isPressed Keyboard.Control keyboardModel

                ctl_forward =
                    Keyboard.isPressed Keyboard.ArrowRight keyboardModel
                        && Keyboard.isPressed Keyboard.Control keyboardModel
            in
            if shift_enter then
...
goNextDn to_id model =
    let
        new_eq =
            Tree.find to_id model.equation_edits_tree
    in
    { model
...
goBackUp model =
    let
        new_eq =
            Tree.findParentNode (newStep 0) model.current_equation model.equation_edits_tree
    in
    { model
        | current_equation = new_eq
        , laTeX = new_eq.eq
...
enterAndGo model keyboardModel next_id =
    let
        new_tree =
            Tree.insertUnder model.current_equation (newStep2 next_id model.laTeX) Tree.After model.equation_edits_tree

        new_current_eq =
            Tree.find next_id new_tree
    in
    { model
